      ...
      ImportDeclaration: function () {
        function ImportDeclaration(path, state) {
          ...
          var _state$opts = state.opts,
              ...
              statefulComponent = _state$opts.statefulComponent;
          var file = state.file;
          ...
          if ((0, _path.extname)(importPath) === '.svg') {
            ...
            if (svgCode.openingElement.attributes.length > 1) {
              ...
              if (statefulComponent) {
                svgCode.openingElement.attributes.push(t.jSXAttribute(t.jSXIdentifier('ref'), t.jSXExpressionContainer(t.arrowFunctionExpression([t.identifier('el')], t.callExpression(t.identifier('refcb'), [t.identifier('el')])))));
              }
            ...
            if (opts.SVG_DEFAULT_PROPS_CODE) {
              var svgReplacement = statefulComponent ? buildSvgWithDefaultsSF(opts) : buildSvgWithDefaults(opts);
              path.replaceWithMultiple(svgReplacement);
            } else {
              var _svgReplacement = statefulComponent ? buildSvgSF(opts) : buildSvg(opts);
